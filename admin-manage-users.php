...
<?php
  ...
  if (isset($_GET['q'])) {
    ...
    $condition .= " AND LOWER(CONCAT(username, student_no)) LIKE LOWER('%$q%')";
  }
?>
...
<?php
  ...
  $sql = mysqli_query($conn, $normal_query) or die(mysql_error());
?>


<form action="admin-action-user.php" method="post" class="form-fab">
...
?>
  <tr>
    ...
    <td>
      <div>
        <form action="admin-action-user.php" method="post">
          <input type="hidden" name="uid" value="<?php echo $row['id']; ?>" />
          <input type="hidden" name="is_admin" value="<?php echo $admin; ?>" />
          
          <label class="mdl-icon-toggle mdl-js-icon-toggle mdl-js-ripple-effect" for="submit-icon-<?php echo $row['id']; ?>">
            <input type="submit" id="submit-icon-<?php echo $row['id']; ?>" name="modify-user"
